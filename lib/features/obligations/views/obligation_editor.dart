...
class ObligationEditor extends StatefulWidget {
  final String? id;
  final bool readOnly;
  const ObligationEditor({super.key, this.id, this.readOnly = false});

  @override
  State<ObligationEditor> createState() => _ObligationEditorState();
}

class _ObligationEditorState extends State<ObligationEditor> {
  final _form = GlobalKey<FormState>();
  final FormData _d = {};

  void handleMoreTap(BuildContext context) async {
    Navigator.pushNamed(context, "/obligations/${widget.id!}/menu");
  }
  ...
  void handleSubmit() {
    ...
    if (_form.currentState!.validate()) {
      ...
      Future(() async {
        if (widget.id == null) {
          await obligationProvider.create(
            fee: _d["fee"],
            amount: _d["amount"],
        ...
        if (widget.id != null) {
          await obligationProvider.update(
            widget.id!,
            fee: _d["fee"],
            amount: _d["amount"],
            issuedAt: _d["issuedAt"].dateTime,
        ...
        alert(messenger, "Edit obligation details failed");
      });
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: FutureBuilder(
            future: Future.wait([
              ...
              if (widget.id != null) obligationProvider.get(widget.id!),
            ]),
            builder: (context, snapshot) {
              ...
              final obligation = widget.id != null
                  ? (snapshot.data![4] as Obligation)
                  : null;
                ...
                child: Column(
                  ...
                  children: [
                    AmountFormField(
                      ...
                      initialValue:
                          _d["amount"] ?? obligation?.amount.abs(),
                    ...
                    AmountFormField(
                      readOnly: widget.readOnly,
                      initialValue: _d["fee"] ?? obligation?.feeAmount,
                      decoration: InputStyles.field(
                        hintText: "Enter fee...",
                        labelText: "Fee",
                      ),
                    ...
                    SelectFormField<EntryType>(
                      readOnly: widget.readOnly,
                      initialValue: _d["type"] ?? obligation?.entryType,
                      ...
                      decoration: InputStyles.field(
                        labelText: "Type",
                        hintText: "Select obligation type...",
                      ),
                    ),
                    SelectFormField<ObligationStatus>(
                      readOnly: widget.readOnly,
                      ...
                      options: ObligationStatus.values.map((v) {
                        return SelectItem(value: v, label: v.label);
                      }).toList(),
                      decoration: InputStyles.field(
                        labelText: "Status",
                    ...
                    SelectFormField<String>(
                      readOnly: widget.readOnly,
                      initialValue:
                          _d["categoryId"] ?? obligation?.categoryId,
                    ...
                    WhenFormField(
                      ...
                      initialValue:
                          _d["issuedAt"] ??
                          (obligation?.issuedAt != null
                              ? When.specificTime(obligation!.issuedAt)
                              : When.now()),
                    ...
                    SelectFormField<String>(
                      readOnly: widget.readOnly,
                      initialValue:
                          _d["journalId"] ?? obligation?.journalId,
                    ...
                    MultiSelectFormField<String>(
                      readOnly: widget.readOnly,
                      initialValue:
                          _d["labelIds"] ?? obligation?.labelIds ?? [],
